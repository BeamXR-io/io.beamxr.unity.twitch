...
namespace BeamXR.Streaming.Twitch
{
    internal partial class TwitchConnection
    {
        ...
        private int _sessionRandom = DateTime.Now.Second;

        private void ReadThreadLoop()
        {
            BeamLogger.LogDev($"{IRCMessageTag.Thread()} Read thread started");
            ...
            BeamLogger.LogDev($"{IRCMessageTag.Thread()} Read thread stopped");
        }
        ...
        {
            BeamLogger.LogDev($"{IRCMessageTag.Read()} {raw}");
        ...
        private void HandlePRIVMSG(string ircString, string tagString)
        {
            ...
            if (tags.colorHex.Length <= 0)
                tags.colorHex = ChatColors.GetRandomNameColor(_sessionRandom, login);
            ...
            chatterQueue.Enqueue(new ChatMessage(login, channel, message, tags));
        }
