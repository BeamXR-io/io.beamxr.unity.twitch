...
namespace BeamXR.Twitch
{
    ...
    /// </summary>
    public class EmoteDownloader : MonoBehaviour
    {
        [SerializeField, ReadOnly] Texture2D _sheetTexture;
        [SerializeField] Sprite _spriteSheet;
        ...
        [SerializeField] MeshRenderer _emotePreview;

        [SerializeField] TwitchChatWindow _chatWindow;
        ...
        IEnumerator FetchEmoteData(string accessToken, string clientID, string channelName = null)
        {
            ...
            {
                Debug.LogError($"Error {webRequest.responseCode} when getting global emote data");
                yield break;
            }
            ...
            if (response == null)
            {
                Debug.LogError($"Unexpected response from global emote list GET request: {json}");
                yield break;
            }
            ...
            if (channelName != null)
            {
                ...
                yield return GetUserData(channelName, accessToken, clientID, userJson =>
                {
                    ...
                    {
                        Debug.LogError($"Unexpected response from channel emote list GET request: {json}");
                    }
                    else
                    {
        ...
        IEnumerator GetUserData(string channelName, string accessToken, string clientID, Action<string> callback)
        {
            ...
            if (webRequest.responseCode != 200)
            {
                Debug.LogError($"Error {webRequest.responseCode} when fetching user data for {channelName}.");
                yield break;
            }
        ...
        IEnumerator GetChannelEmoteList(string channelName, string accessToken, string clientID, string broadcasterId, Action<string> callback)
        {
            ...
            if (webRequest.responseCode != 200)
            {
                Debug.LogError($"Error {webRequest.responseCode} when fetching emote list for channel {channelName}.");
                yield break;
            }
        ...
        public IEnumerator DownloadAllEmotes(Dictionary<string, string> emoteUrls)
        {
            ...
            foreach (var emote in emoteUrls)
            {
                ...
                if (webRequest.responseCode != 200)
                {
                    Debug.LogError($"Couldn't download image from {emote.Value}");
                    continue;
                }
                ...
                //Debug.Log($"Trying to create sprite {i}, dimensions {tex.width},{tex.height} pos {x},{y}");
                try
                {
                ...
                {
                    Debug.LogError($"Sprite size problem at {emote.Key}");
                }
                _sheetTexture.Apply();

                if (_emotePreview != null)
                {
                    _emotePreview.material.mainTexture = _sheetTexture;
                }
                //Debug.Log($"Created sprite {i}");
                i++;
            }
            _sheetTexture.Apply();
